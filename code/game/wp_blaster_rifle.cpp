...
//---------------------------------------------------------
void WP_FireEE3CarbineRifleMissile(gentity_t *ent, vec3_t start, vec3_t dir, qboolean altFire)
//---------------------------------------------------------
{
	int velocity = BLASTER_VELOCITY;
	int	damage = altFire ? weaponData[WP_EE3_CARBINE_RIFLE].altDamage : weaponData[WP_EE3_CARBINE_RIFLE].damage;
	...
	missile->classname = "blaster_proj";
	missile->s.weapon = WP_EE3_CARBINE_RIFLE;
...
//---------------------------------------------------------
{
	...
	if (cg.zoomMode >= ST_A280)
	{
		...
		WP_FireEE3CarbineRifleMissile( ent, ent->client->renderInfo.eyePoint, forwardVec, alt_fire );
	}
	else
	{
		AngleVectors( angs, dir, NULL, NULL );
